...
def df_log_to_receipt(row, contract_obj, event):
    ...
    # Create logs list with a single log entry as AttributeDict
    logs = [
        AttributeDict({
            'blockHash': HexBytes(row['block_hash']),
    ...
    # Create the receipt dictionary as AttributeDict
    receipt = AttributeDict({
        'blockHash': HexBytes(row['block_hash']),
        'blockNumber': row['block_number'],
        'contractAddress': HexBytes(address),
        'transactionHash': HexBytes(transaction_hash),
        'transactionIndex': transaction_index,
        'logs': logs
    })
